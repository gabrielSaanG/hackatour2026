...
function encodeRunBitpacked(values, opts) {
  ...
  return Buffer.concat([
    Buffer.from(import_varint.default.encode(values.length / 8 << 1 | 1)),
    buf
  ]);
}
...
async function parseParquetFile(file, options) {
  ...
  for await (const rowBatch of rowBatches) {
    ...
    for (const row of rowBatch) {
      if ((options == null ? void 0 : options.limit) && rows.length >= ((_b = options == null ? void 0 : options.core) == null ? void 0 : _b.limit)) {
    ...
    if (limitHasReached) {
      import_log.default.warn(`Rows number limit has been reached. Only first ${(_c = options == null ? void 0 : options.core) == null ? void 0 : _c.limit} are loaded`)();
      break;
    }
  }
...
async function encodeArrowToParquet(table, options) {
  ...
  try {
    ...
    const bytes = new Uint8Array(parquetBytes.buffer, parquetBytes.byteOffset, parquetBytes.byteLength);
